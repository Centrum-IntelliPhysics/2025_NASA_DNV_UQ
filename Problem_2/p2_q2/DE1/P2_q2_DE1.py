...
class ReliabilityBasedOptimizer:
    """
    Implements reliability-based design optimization for the NASA/DNV UQ Challenge.
    
    This solves Question 2: Find the control variable X_c that minimizes the greatest value
    that the system failure probability might take as X_e varies in the epistemic uncertainty space.
    ...
    def __init__(self, exe_path, data_path, n_epistemic_samples=35, 
                 checkpoint_dir="reliability_checkpoints", max_runtime_hours=48):
        ...
        self.best_solution = {
            ...
            "worst_pof": 1.0,  # Starting with worst case (100% failure)
            "iteration": 0
        }
        ...
        print(f"ReliabilityBasedOptimizer initialized with all aleatory samples and {n_epistemic_samples} epistemic samples")
    ...
    def load_data(self):
        ...
        xa1 = np.load(self.data_path / "Xa1_stratified_samples_2000.npy")
        xa2 = np.load(self.data_path / "Xa2_stratified_samples_2000.npy")
    ...
    def run_forward_model(self, input_data):
        ...
        try:
            ...
            result = subprocess.run(command, capture_output=True, text=True)  # 5-minute timeout
    ...
    def evaluate_reliability(self, Xa, Xe, Xc, seeds=None):
        """
        Evaluate the system failure probability for given inputs.
        ...
        if Y_out is None:
            return np.nan, [np.nan] * len(self.I2)
    ...
    def save_checkpoint(self, final=False):
        ...
        results = {
            ...
            "best_pof": float(self.best_solution["worst_pof"]),
        ...
        if len(self.iteration_history) > 0:
            ...
            self.plot_optimization_progress(save_path=self.checkpoint_dir / "reliability_optimization_progress.png")
        
        print(f"\n=== CHECKPOINT SAVED ===")
        print(f"Saved to: {versioned_checkpoint_file}")
        print(f"Iteration: {self.best_solution['iteration']}")
        print(f"Best worst-case PoF: {self.best_solution['worst_pof']:.6f}")
    ...
    def restore_checkpoint(self):
        ...
        try:
            ...
            print(f"Best worst-case PoF so far: {self.best_solution['worst_pof']:.6f}")
    ...
    def plot_optimization_progress(self, save_path=None):
        ...
        # System failure probability plot
        ax.plot(self.iteration_history, self.objective_history, 'r-s', label='Worst-Case System Failure Probability')
        ax.set_xlabel('Iteration')
        ax.set_ylabel('Probability of Failure')
        ax.set_title('Reliability-Based Optimization Progress')
        ax.grid(True)
        ax.legend()
    ...
    def reliability_based_design(self, hyperparam_set=None, max_iter=20, verbose=True):
        """
        Solve the reliability-based design optimization problem:
        Find X_c that minimizes max(X_e∈E) pof_sys(X_e, X_c)
        ...
        """
        if verbose:
            print("Solving Reliability-Based Design Optimization Problem...")
        ...
        # Define the objective function: minimize worst-case system failure probability
        def objective_function(X_c):
            ...
            if X_c_tuple in self.high_fidelity_cache:
                ...
                return worst_pof  # Return directly for minimization
            ...
            # Find the worst-case system failure probability across all epistemic samples
            ...
            # Evaluate system failure probability for each epistemic sample
            for i, X_e in enumerate(E_subset):
                # Evaluate reliability
                pof_sys, _ = self.evaluate_reliability(Xa_subset, X_e, X_c)
            ...
            if is_high_fidelity:
                ...
                if verbose and self.func_eval_count % 10 == 0:
                    print(f"X_c: {X_c}, HIGH-FIDELITY evaluation - worst-case pof: {worst_case_pof:.6f}")
            elif verbose and self.func_eval_count % 20 == 0:
                print(f"X_c: {X_c}, worst-case pof: {worst_case_pof:.6f}")
            
            # Check if this solution is better than the current best
            if worst_case_pof < self.best_solution["worst_pof"]:
                # Update best solution
                self.best_solution["X_c"] = X_c.copy()
                self.best_solution["worst_pof"] = worst_case_pof
                
                if verbose and self.func_eval_count % 10 == 0:
                    print(f"New best solution from func eval! Worst-case PoF: {worst_case_pof:.6f}")
            ...
            if self.timeout_mgr.should_checkpoint():
                if X_c_tuple not in self.eval_cache and len(self.iteration_history) > 0:
                    ...
                    self.objective_history.append(worst_case_pof)
                    
                self.save_checkpoint()
            
            # Return worst-case system failure probability (to be minimized)
            return worst_case_pof
        ...
        def callback(xk, convergence=None):
            ...
            if X_c_tuple in self.high_fidelity_cache:
                ...
                for X_e in E_subset:
                    # Evaluate reliability
                    pof_sys, _ = self.evaluate_reliability(Xa_subset, X_e, current_X_c)
            ...
            self.iteration_history.append(iteration_counter)
            self.objective_history.append(current_pof)
            
            # Update best solution if better
            if current_pof < self.best_solution["worst_pof"]:
                self.best_solution["X_c"] = current_X_c.copy()
                self.best_solution["worst_pof"] = current_pof
                self.best_solution["iteration"] = iteration_counter
                
                if verbose:
                    print(f"New best solution found! Worst-case PoF: {current_pof:.6e}")
            
            # Always print progress for each iteration
            if verbose:
                elapsed = self.timeout_mgr.elapsed_time_formatted()
                print(f"Iteration {iteration_counter}: Worst-case PoF = {current_pof:.6e} " +
        ...
        try:
            ...
            if len(self.iteration_history) > 0:
                ...
                print(f"{'Iteration':<10}{'Worst-case PoF':<15}")
                print("-"*25)
                for i, (iter_num, pof) in enumerate(zip(self.iteration_history, self.objective_history)):
                    print(f"{iter_num:<10}{pof:<15.6e}")
            ...
            return self.best_solution["X_c"], self.best_solution["worst_pof"]
...
def main():
    ...
    results_dir = Path("reliability_results")
    results_dir.mkdir(exist_ok=True)
    
    # Define maximum runtime in hours
    max_runtime_hours = 72  # 72 hours = 3 days
    
    print("NASA/DNV UQ Challenge - Reliability-Based Design Optimization")
    print("=" * 80)
    print(f"Maximum runtime: {max_runtime_hours} hours")
    print(f"Started at: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Set epistemic sample size
    n_epistemic = 20  # Number of epistemic samples
    
    print(f"Using all available aleatory samples and {n_epistemic} epistemic samples")
    
    # Define hyperparameter set
    hyperparam_set = {
        'strategy': 'best1bin',  # Random base vector selection for better exploration
        'mutation': (0.5, 1.0),         # Slightly increased mutation rate for better exploration
        ...
        'maxiter': 15,           # Keep polishing enabled
    }
    ...
    optimizer = ReliabilityBasedOptimizer(
        exe_path=exe_path, 
        data_path=data_path, 
    ...
    print("\nStarting reliability-based design optimization...")
    X_c_optimal, worst_pof = optimizer.reliability_based_design(
    ...
    if X_c_optimal is not None:
        ...
        # Save to files
        np.save(results_dir / "reliability_based_results.npy", result_info)
        
        # Also save in JSON format for better readability
        with open(results_dir / "reliability_based_results.json", "w") as f:
            json.dump(result_info, f, indent=4)
        
        print(f"\nResults saved to {results_dir / 'reliability_based_results.json'}")
        
        # Report final status
        print(f"\nOptimization {completion_status}")
        print(f"X_c = {X_c_optimal}")
        print(f"Worst-case system failure probability = {worst_pof:.6e}")
    
    total_time = time.time() - optimizer.timeout_mgr.start_time
    print(f"\nTotal runtime: {datetime.timedelta(seconds=int(total_time))}")
    print("Reliability-based design optimization process completed.")
